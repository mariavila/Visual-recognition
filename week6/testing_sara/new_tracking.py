...
#        frame_n_bb = remove_overlaps(frame_n_bb)
        ...
        if frame > 0 and mode == 'of':
            #past frame
            first_frame = cv2.imread((frames_path + '/{:06d}.png').format(frame-1))
            first_frame = cv2.cvtColor(first_frame, cv2.COLOR_BGR2GRAY)

            #actual frame
            second_frame = cv2.imread((frames_path + '/{:06d}.png').format(frame))
            ...
            for j, frame in enumerate(nframes):
                if j+1 == len(nframes):
                    ...
                    if nframes[j+1] > nframes[j]+1:
                        ...
                        first_frame = cv2.imread((frames_path + '/{:06d}.png').format(frame))
                        first_frame = cv2.cvtColor(first_frame, cv2.COLOR_BGR2GRAY)
                        
                        #I don't have the bb info of this frame
                        second_frame = cv2.imread((frames_path + '/{:06d}.png').format(frame+1))
                        second_frame = cv2.cvtColor(second_frame, cv2.COLOR_BGR2GRAY)
